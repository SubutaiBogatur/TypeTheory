...
let rec subst lm v_old theta =
	if d () then print_string ("subst: " ^	(string_of_lambda lm) 
			^ " " ^ v_old ^ " " ^ (string_of_lambda theta) ^ "\n"); 
	match lm with 
...
print_string (string_of_lambda (normal_beta_reduction (lambda_of_string "(\\x.x)y1")))
